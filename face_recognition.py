...
class FaceRecognition:
    def __init__(self,root):
        ...
        img_top = img_top.resize((765,720))
        self.photoimg_top = ImageTk.PhotoImage(img_top)

        f_lbl1 = Label(self.root,image = self.photoimg_top)
        f_lbl1.place(x=0,y=70,width=765,height = 720)

        #Image--2
        img_top_2 = Image.open(r"college_images/facial_recognition_system_identification_digital_id_security_scanning_thinkstock_858236252_3x3-100740902-large.jpg")
        img_top_2 = img_top_2.resize((765,720))
        self.photoimg_top_2 = ImageTk.PhotoImage(img_top_2)

        f_lbl1 = Label(self.root,image = self.photoimg_top_2)
        f_lbl1.place(x=765,y=70,width=765,height = 721)

        #Button
        b7 = Button(f_lbl1,text="Face Recognition",cursor="hand2",font=("times new roman",15,"bold"),bg="red",fg="white",command=self.face_recog)
        b7.place(x=283,y=610,width=200,height= 50)

    #---------------------Attendance-----------
    def mark_attendance(self,i,r,n,d):
        with open("krish.csv","r+",newline="\n") as f:
            myDataList = f.readlines()
            name_list=[]
            for line in myDataList:
                entry=line.split(",")      #krish,2,id,.....
